...
/** 标题清洗：去首尾引号 + 截断到 MAX_TITLE_LENGTH，空串返回 null */
function cleanTitle(raw: string): string | null {
  const cleaned = raw.trim().replace(/^["'""''「《]+|["'""''」》]+$/g, '').trim()
  return cleaned.slice(0, MAX_TITLE_LENGTH) || null
}

/** 常见礼貌/动作前缀（本地启发式回退时剥离，单字动作词风险高已排除） */
const TITLE_POLITE_PREFIXES: readonly RegExp[] = [
  /^(请|麻烦你?|帮(我|忙)|能不能|能否|可不可以|可以帮我|能帮我|给我|我(想|要|希望|需要)|想要)/,
  /^(帮我)?(查一下|查查|查询|查看|看一下|看看|搜索一下|搜索|检查一下|检查|获取|了解一下|了解|告诉我)/,
  /^(please|pls|help me|can you|could you|would you|i (want|need) to)\s*/i,
]

/**
 * 本地启发式标题：取首个非空行，剥离常见礼貌/动作前缀后清洗
 *
 * 仅用于 CLI LLM 标题失败时的回退，best-effort，不追求语义完美。
 */
function localHeuristicTitle(userMessage: string): string | null {
  const firstLine = userMessage.split('\n').map((line) => line.trim()).find((line) => line.length > 0) ?? ''
  let s = firstLine.replace(/^["'""''「《]+/, '').trim()
  // 反复剥离堆叠前缀（如"帮我"+"查一下"），最多 4 轮，避免过度裁剪
  for (let i = 0; i < 4; i++) {
    let changed = false
    for (const re of TITLE_POLITE_PREFIXES) {
...
export class AgentOrchestrator {
  ...
  async generateTitle(input: AgentGenerateTitleInput): Promise<string | null> {
    ...
    try {
      ...
      // 仅 Agent 模式的供应商（如 Cursor）无 HTTP 标题端点：
      // 优先用 CLI 做一次性 LLM 摘要，与其他渠道体验一致；失败/超时再回退本地启发式
      if (isAgentOnlyProvider(channel.provider)) {
        if (channel.provider === 'cursor') {
          try {
            ...
            console.warn('[Agent 标题生成] Cursor CLI 标题为空，回退本地启发式')
          } catch (error) {
