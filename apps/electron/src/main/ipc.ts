...
import { runAgent, stopAgent, generateAgentTitle, saveFilesToAgentSession, saveFilesToWorkspaceFiles, isAgentSessionActive, queueAgentMessage, updateAgentPermissionMode, rewindAgentSession, remoteBridge } from './lib/agent-service'
import { remoteAuth } from './lib/remote-auth'
...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle(
    REMOTE_BRIDGE_IPC_CHANNELS.GENERATE_PAIRING_CODE,
    async (_, gatewayUrl: string): Promise<{ code: string; qrContent: string; token: string }> => {
      return remoteAuth.generatePairingCode(gatewayUrl)
    }
  )

  ipcMain.handle(
    REMOTE_BRIDGE_IPC_CHANNELS.CONNECT,
    async (_, gatewayUrl: string, token: string, code: string): Promise<void> => {
      await remoteBridge.connect(gatewayUrl, token, code)
    }
  )

  ipcMain.handle(
    REMOTE_BRIDGE_IPC_CHANNELS.DISCONNECT,
    async (): Promise<void> => {
  ...
  ipcMain.handle(
    REMOTE_BRIDGE_IPC_CHANNELS.GET_STATUS,
    async (): Promise<{ connected: boolean; gatewayUrl: string | null }> => {
      return { connected: remoteBridge.isConnected, gatewayUrl: null }
    }
  )
}
