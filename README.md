...
4. Agent 模式支持 Anthropic 协议或 Anthropic 兼容协议渠道（如 Anthropic、DeepSeek、Kimi API、Kimi Coding Plan），以及 Cursor (CLI) 渠道（通过本机 cursor-agent 运行，仅需 API Key）。
...
| Cursor (CLI) | 暂不支持 | 支持 | 本机 cursor-agent CLI（headless），自动安装 |

> **Kimi Coding Plan 用户须知**：Proma 已获得 Kimi 官方白名单支持，使用 Proma 连接 Kimi Coding Plan 不会触发第三方客户端封号策略，可放心使用。

Agent 模式默认使用 Claude Agent SDK，因此 Anthropic 或 Anthropic 兼容协议渠道可直接用于 Agent；此外还支持 Cursor (CLI) 渠道——它不走 Anthropic 协议，而是通过本机 `cursor-agent` 的 headless 模式运行（详见下方“Cursor (CLI) 渠道”）。Chat 模式则通过 `@proma/core` 的 Provider Adapter 统一接入不同协议。

### Cursor (CLI) 渠道

Cursor 渠道让 Agent 模式直接复用 Cursor 订阅的全部 agentic 能力（文件编辑、Shell、代码检索等），底层通过本机 `cursor-agent` CLI 的 headless 模式运行，由 Proma 统一编排（持久化、事件流、渲染与 Claude 渠道一致）。

- **自动安装**：首次使用时 Proma 调用 Cursor 官方脚本将 `cursor-agent` 安装到 `~/.local/bin`（不随应用分发二进制）；已安装则自动复用，可通过 `cursor-agent update` 升级。
- **配置**：在 **设置 > 渠道** 新建 Cursor 渠道，仅需填写 Cursor API Key（经 `CURSOR_API_KEY` 注入子进程）；无需 Base URL，模型列表来自 `cursor-agent --list-models`。
- **权限模式**：Plan 模式映射为 `--mode plan`（只读规划），其它模式使用 `--force --trust` 自动放行。
- **Plan 模式闭环**：规划回合结束后自动弹出计划审批横幅；批准后自动发送执行指令并以 `--force` 启动执行回合（详见 `docs/cursor-cli-plan-mode.md`）。

**已知限制**：

- 仅用于 **Agent 模式**，不在 Chat 模式中出现。
- 单轮 headless 进程：暂不支持流式追加消息、软中断；权限模式切换在**下一条消息**生效。
- 不上报 token 用量（CLI 不输出 token 计数），因此 Cursor 会话不显示上下文 / token 统计。
- 暂不支持 MCP：Proma 内置工具（记忆 / 定时任务 / 生图 / 飞书）为 SDK 进程内工具，无法注入到 cursor-agent。
...
| 包 | 版本 | 职责 |
| --- | --- | --- |
| `@proma/electron` | `0.12.1` | Electron 桌面应用 |
| `@proma/shared` | `0.1.29` | 共享类型、IPC 常量、配置和工具 |
